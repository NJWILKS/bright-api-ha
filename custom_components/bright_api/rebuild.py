...
def _month_keys(start: datetime, end: datetime) -> list[str]:
    """Return UTC month keys covering a stored ledger range, inclusively."""
    current = start.astimezone(UTC).replace(day=1, hour=0, minute=0, second=0, microsecond=0)
    last = end.astimezone(UTC).replace(day=1, hour=0, minute=0, second=0, microsecond=0)
    months: list[str] = []
    while current <= last:
...
async def async_clear_interval_history(hass: HomeAssistant, entry_id: str) -> None:
    """Delete only raw/tariff Store data owned by one Bright config entry."""
